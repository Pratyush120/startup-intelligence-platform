...
class ExecutiveReport:
    def generate(self, events, company_store):
        ...
        report["funding_events"] = sum(e.event_type == "Funding" for e in events)

        report["hiring_events"] = sum(e.event_type == "Hiring" for e in events)

        report["layoff_events"] = sum(e.event_type == "Layoff" for e in events)

        report["expansion_events"] = sum(e.event_type == "Expansion" for e in events)

        report["acquisition_events"] = sum(
            e.event_type == "Acquisition" for e in events
        )
        ...
        publishers = Counter(e.source for e in events)
