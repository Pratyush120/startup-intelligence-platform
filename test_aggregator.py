...
async def test_aggregator():
    ...
    if ctx.get("latest_news"):
        print(f"Top News: {ctx['latest_news'][0].title} (Source: {ctx['latest_news'][0].source.provider})")

    print("\n--- GEMINI REASONING ---")
    gemini = GeminiProvider()
    res = gemini.analyze_strategic_context(ctx, "What are the latest strategic moves and financial health of Apple?")
    print(res.summary)
